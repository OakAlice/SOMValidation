# Process the raw dog data in alignment with the quoll data
...
compute_features <- function(dat) {
  ...
  skx<-skewness(dat$ANeck_x)
  sky<-skewness(dat$ANeck_y)
  skz<-skewness(dat$ANeck_z)
...
# View the transformed dataset # just to check
